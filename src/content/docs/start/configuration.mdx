...
of `language`, `parser`, `format`, and `formatter` when present.

:::

### Discovery

BeDoc will automatically discover and load parsers and formatters based on the
...
local project directory. When using the `language` and `format` options, BeDoc will attempt to match parser and formatter actions in these locations.

Read more about [Discovery](/start/discovery/).

### Mock Mode

When a `mock` path is provided, BeDoc will only use mock parsers and formatters
...
detail. These settings control BeDoc's own internal logging only — actions and
hooks are not given a logger.
